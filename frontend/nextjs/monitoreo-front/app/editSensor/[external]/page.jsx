...
import { enviar, obtener, obtenerTodo } from "@/hooks/Conexion";
...
export default function Page({ params }) {
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            ...
            const response = await obtenerTodo(`obtener/sensor/${external}`, token);
            console.log(response);
            setValue('nombre', response.data.nombre);

            // Dividir la cadena IP:Puerto en dos partes
            const partes = response.data.ip.split(':');

            // Establecer la IP y el puerto en los campos del formulario
            setValue('ip', partes[0]);
            setValue('puerto', partes[1]);
        };
